...
const addPurchases = (arr) => {
  let priceArr=[];
   arr.map(element => {
     priceArr.push(element.purchasePrice);
  })
  let sum=priceArr.reduce( (previousValue, currentValue) => previousValue + currentValue, 0)
  return sum;
};
...
const countNumberOfElements = (arr) => {
 const counter = arr.reduce((counter,element)=>{
...
const returnNames = (arr) => {
  let namesArr=arr.reduce((names, e) => {
    names.push(e.name);
    return names;
  }, [])
  return namesArr;
...
const reversedString = (str) => {
  let reverse = str.split('').reduce((a, b) => {
    a.unshift(b);
    return a;
  }, [])
  return reverse.join('');
